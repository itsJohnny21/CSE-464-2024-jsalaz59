...
class DOTGenerator():
    ...
    def delete_files(self, filename:str):
        if os.path.exists(self.directory):
            ...
            for subdirectory in os.listdir(valid_path):
                if subdirectory.startswith(filename):
                    for file in os.listdir(os.path.join(self.directory, "valid", subdirectory)):
                        os.remove(os.path.join(self.directory, "valid", subdirectory, file))
                        
        if os.path.exists(self.directory):
            invalid_path = os.path.join(self.directory, "invalid")
            for subdirectory in os.listdir(invalid_path):
                if subdirectory.startswith(filename):
                    for file in os.listdir(os.path.join(self.directory, "invalid", subdirectory)):
                        os.remove(os.path.join(self.directory, "invalid", subdirectory, file))
    ...
    def generate_file(self, filename:str="", graph:dict=None, graph_options:str="d", node_options:str="a10", edge_options:str="l6", error_options="", with_answers:bool=False, invalid=False) -> None:
        ...
        if not invalid:
            ...
            if with_answers and not invalid:
                delimiter = ", "
                edges = "\n".join([self.format_edge(edge) for edge in graph['edges']]) + "\n"
                nodes = "\n".join([str(node) for node in graph['nodes']]) + "\n"
                node_labels = "\n".join(f'{node}{delimiter}{graph["nodes"][node]["label"] if "label" in graph["nodes"][node] else ""}' for node in graph['nodes']) + "\n"
