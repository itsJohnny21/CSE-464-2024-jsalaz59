    ...
    def generate_file(self, filename:str="", graph:dict=None, graph_options:str="d", node_options:str="a10", edge_options:str="l6", error_options="", with_answers:bool=False, invalid=False) -> None:
        ...
        if not invalid:
            ...
            if with_answers and not invalid:
                ...
                edge_labels = "\n".join(f'{self.format_edge(edge)}{delimiter}{graph["edges"][edge]["label"] if "label" in graph["edges"][edge] else ""}' for edge in graph['edges']) + "\n"
                edge_weights = "\n".join(f'{self.format_edge(edge)}{delimiter}{graph["edges"][edge]["weight"] if "weight" in graph["edges"][edge] else ""}' for edge in graph['edges']) + "\n"
